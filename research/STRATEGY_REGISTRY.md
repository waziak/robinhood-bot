...
Generated 2026-09-13 20:08 UTC by `python -m research.registry`. States: IDEA, RESEARCHING, FAILED, SHADOW_CANDIDATE, SHADOW_VALIDATED, LIVE_ELIGIBLE, RETIRED.

Summary: RESEARCHING=2, FAILED=9, RETIRED=3
...
| monthly_sma10_timing_multi | daily/monthly | **FAILED** | 443 | reality check: test-window gains are explained by market drift/exposure (62nd percentile vs. random-entry null), not timing skill |
...
- **SHADOW_CANDIDATE** (automatic, this script): passed the pre-registered validation gate AND beats a random-entry null at ≥95th percentile in the sealed test window (≥80th in validation too, so it is not a one-window fluke). Nothing currently qualifies.
